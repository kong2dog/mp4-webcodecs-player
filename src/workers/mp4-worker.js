...
function createMP4Box() {
    ...
    mp4boxfile.onReady = (info) => {
      ...
      if (videoTrack) {
        mp4boxfile.setExtractionOptions(videoTrack.id, 'video', { nbSamples: 1000 });
        
        const videoDesc = videoTrack.codec.startsWith('vp08') ? 'vp8' : videoTrack.codec;
      ...
      if (audioTrack) {
        mp4boxfile.setExtractionOptions(audioTrack.id, 'audio', { nbSamples: 1000 });
        ...
        if(audioDecoder.state === 'unconfigured') audioDecoder.configure(config);
      }
      ...
      self.postMessage({
        ...
        info: {
          ...
          videoTrack: videoTrack ? { width: videoTrack.video.width, height: videoTrack.video.height } : null
        }
      });
...
function performSeek(timeSec) {
    ...
    // We need to recreate mp4box to reliably re-parse from a seek point?
    // Actually mp4box seek works by setting the next sample index.
    // But we need to make sure decoders are re-configured.
    
    // Simpler approach: Re-configure using existing tracks
    if (videoTrack) {
        const config = {
            codec: videoTrack.codec,
            codedWidth: videoTrack.video.width,
